...
class AttLayer(Layer):
    ...
    def call(self, x, mask=None):
        eij = K.tanh(tf.matmul(x, self.W))
        ai = K.exp(eij)
        weights = ai / tf.reduce_sum(ai, axis=1).dimshuffle(0, 'x')
...
class Diagonal(Layer):
    ...
    def call(self, x, mask=None):
        ...
        mult = x * kernel
        mult = tf.reshape(mult, (-1, int(self.n_inputs_per_node)))
...
class SpraseLayerWithConnection(Layer):
    ...
    def call(self, x, mask=None):
        ...
        for i in range(n_outputs):
            ...
            if self.use_bias:
                y0 = tf.nn.bias_add(y0, self.bias[i])
        ...
        output = tf.reshape(output, (-1, self.units))
...
class L1L2_with_map(Regularizer):
    ...
    def __call__(self, x):
        ...
        regularization = 0.
        if self.l1:
            regularization += tf.reduce_sum(self.l1 * K.abs(x_masked))
        if self.l2:
            regularization += tf.reduce_sum(self.l2 * K.square(x_masked))
